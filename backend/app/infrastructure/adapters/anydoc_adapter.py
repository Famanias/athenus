...
class AnyDocDocumentParsingAdapter(IDocumentParsingCapability):
    ...
    def _parse_with_anydoc(self, request: DocumentParsingRequest, file_format: str) -> DocumentParsingResponse:
        ...
        # Sanitize markdown content to remove any embedded raw PDF/XMP metadata stream blocks
        clean_markdown_lines = []
        in_xmp = False
        for line in markdown_content.splitlines():
            if "<?xpacket" in line or "<x:xmpmeta" in line or "<rdf:RDF" in line:
                in_xmp = True
                continue
            if "<?xpacket end" in line or "</x:xmpmeta>" in line or "</rdf:RDF>" in line:
                in_xmp = False
                continue
            if in_xmp:
                continue
            if any(tok in line for tok in ["endstream", "endobj", "obj<<", "/Filter", "/FlateDecode", "xmpMM:", "xapGImg:"]):
                continue
            clean_markdown_lines.append(line)
    ...
    def _parse_with_fallback(self, request: DocumentParsingRequest, file_format: str) -> DocumentParsingResponse:
        """Robust fallback parser for local environments using pypdf for PDFs or UTF-8 text for markdown/txt."""
        pages: List[DocumentPageDTO] = []
        has_scanned = False
        has_tables = False
        markdown_blocks = []

        if file_format == "pdf" or request.file_path.lower().endswith(".pdf"):
            try:
                import pypdf
                ...
                for idx, page in enumerate(reader.pages, start=1):
                    ...
                    for line in extracted.splitlines():
                        ...
                        if in_xmp_block:
                            continue
                        if any(token in line for token in ["endstream", "endobj", "obj<<", "/Filter", "/FlateDecode", "xmpMM:", "xapGImg:", "xml:lang="]):
                            continue
                        clean_lines.append(line)
                    clean_text = "\n".join(clean_lines).strip()

                    is_scanned = len(clean_text) < 20
                    if is_scanned:
                        has_scanned = True
                    ...
                    pages.append(
                        DocumentPageDTO(
                            page_number=idx,
                            text=clean_text if clean_text else f"[Scanned/Image Page {idx}]",
                            page_type="scanned" if is_scanned else "text",
                            section_title=f"Page {idx}",
                        )
                    )
                    markdown_blocks.append(clean_text)
