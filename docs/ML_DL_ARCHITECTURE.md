...
| **Generative LLM Reasoning** | Grounded explanation & citation generation | Autoregressive Decoder Transformers, In-Context Learning (ICL), Temperature/Top-$P$ Sampling | Dynamic Local Ollama Models (`llama3`, `mistral`, `phi3`, etc.), `Groq Llama-3-8192` | `ollama` Scanner, `openai` SDK |
